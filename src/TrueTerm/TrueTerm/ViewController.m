...
- (CGFloat)tell:(char)kind indx:(int)indx {
    if (self.vobj != nil) {
        ...
        if (iidx < self.tabs.count) {
            ...
            CGFloat pada = self.allv.spacing;
    ...
    if (self.vobj != nil) {
        ...
        if ((-1 < indx) && (indx < self.tabs.count)) {
            ...
            NSArray<NSLayoutConstraint *> *cont = @[
                [self.allv.widthAnchor constraintEqualToConstant:wsiz],
                [self.allv.heightAnchor constraintEqualToConstant:hmax],
                [self.tabv.widthAnchor constraintEqualToConstant:wsiz],
                ...
                [self.diva.widthAnchor constraintEqualToConstant:wsiz],
                [self.diva.heightAnchor constraintEqualToConstant:line],
                [self.divb.widthAnchor constraintEqualToConstant:wsiz],
                [self.divb.heightAnchor constraintEqualToConstant:line],
            ];
            ...
            [self show:nil indx:indx sels:0 cidx:-1 rows:self.rows.intValue cols:self.cols.intValue];
            [self show:nil indx:indx sels:1 cidx:-1 rows:self.rows.intValue cols:self.cols.intValue];
...
- (int)show:(NSString *)text indx:(int)indx sels:(int)sels cidx:(int)cidx rows:(int)rows cols:(int)cols {
    ...
    int didx = cidx;
    if (didx < 1) {
        if (sels == 0) { didx = [self.crsa objectAtIndex:indx].intValue; }
        if (sels == 1) { didx = [self.crsb objectAtIndex:indx].intValue; }
        if (sels == 2) { didx = [self.crsc objectAtIndex:indx].intValue; }
    } else {
        if (sels == 0) { [self.crsa replaceObjectAtIndex:indx withObject:@(didx)]; }
        if (sels == 1) { [self.crsb replaceObjectAtIndex:indx withObject:@(didx)]; }
        if (sels == 2) { [self.crsc replaceObjectAtIndex:indx withObject:@(didx)]; }
    }
    ...
    CGFloat fntx = (fntw * (didx % (cols - 1)));
    CGFloat fnty = (fnth * (didx / (cols - 1)));
    int iidx = self.indx.intValue;
    int stat = 0;

    //NSLog(@"SHOW [%d][%d] [%f][%f] [%f][%f] [%f] [%d][%d] [%d][%d] [%d][%ld]",indx,sels,fntx,fnty,fntw,fnth,padl,didx / (cols - 1),didx % (cols - 1),rows,cols,didx, text == nil ? -1 : text.length );
    fntx += padl;
    ...
    if (sels == 0) {
        ...
        if (text == nil) {
            ...
            NSAttributedString *rest = [self.atrs objectAtIndex:indx];
            [stor beginEditing];
            [stor setAttributedString:rest];
            [stor endEditing];
            self.scrl = @0;
        ...
        if (text != nil) {
            ...
            [self.atrs replaceObjectAtIndex:indx withObject:save];
        }
...
- (void)scrollViewDidScroll:(NSNotification *)notification {
    ...
    if ((secs - self.last.intValue) > 1) {
        if ((self.scrl.intValue == 0) && (difp > diff)) {
            self.scrl = @1;
            self.last = @(secs);
        }
        if ((self.scrl.intValue == 1) && (difp < diff)) {
            self.scrl = @0;
            self.last = @(secs);
        }
    }
        ...
        if ((-1 < indx) && (indx < self.tabs.count)) {
            ...
            if ((txtv == txto) && (self.cono != nil)) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    CGFloat txtb = [txtv.layoutManager usedRectForTextContainer:txtv.textContainer].size.height;
                    CGFloat txtc = txtv.textContainerInset.height;
                    CGFloat txtd = 0.00; //self.fnth.floatValue;
...
- (void)tabf:(id)sender {
    ...
    CGFloat wide = self.wide.floatValue;
    CGFloat high = self.high.floatValue;
    if (self.vobj != nil) {
        ...
        if ((indx != iidx) && (indx < self.tabs.count)) {
            ...
            NSTextView *inpt = [self.inpt objectAtIndex:indx];
            if (iidx < 0) { iidx = 0; }
            NSScrollView *oscr = [self.scro objectAtIndex:iidx];
            NSScrollView *iscr = [self.scri objectAtIndex:iidx];
            self.vobj = nil;
            ...
            [self.allv replaceSubview:oscr with:scro];
            [self.allv replaceSubview:iscr with:scri];
            ...
            self.indx = @(indx);
            self.vobj = self.allv;
            [self sets:@"view" wide:wide high:high];
        }
    }
}
...
- (int)newt:(NSString *)text wide:(CGFloat)wide high:(CGFloat)high {
    ...
    [self.tabv insertView:tabb atIndex:(self.tabs.count - 1) inGravity:NSStackViewGravityCenter];
    ...
    if (self.tabs.count > 1) {
        ...
        [self.tabv insertView:imgv atIndex:(self.tabs.count - 1) inGravity:NSStackViewGravityCenter];
    }
    ...
    if (self.vobj == nil) {
        [self.allv setViews:@[self.tabv, self.diva, scro, self.divb, scri] inGravity:NSStackViewGravityBottom];
