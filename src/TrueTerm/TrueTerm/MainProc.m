...
- (int)ginc:(int)mode {
    ...
    if (colv == 0) {
        ...
        if ((rowv == 0) && (mode == 0)) {
            unsigned long maxl = ((self.rows.intValue + 1) * (self.cols.intValue + 1));
            unsigned long maxd = ((self.rows.intValue - 1) * (self.cols.intValue - 1));
            ...
            unsigned char dest[maxl];
            NSLog(@"ANSI SHIF [%ld][%ld] [%ld][%ld]", diff, last, leng, maxd);
            if (leng > maxd) {
                pntr += last;
                bzero(dest, maxl); bcopy(pntr, dest, diff);
                self.ansi = [NSMutableData dataWithBytes:dest length:maxl];
            }
...
- (void)zero:(int)mode {
    ...
    if (mode == 1) { // new + ctc
        ...
        self.ansp = [[NSMutableData alloc] initWithLength:((self.rows.intValue + 1) * (self.cols.intValue + 1))];
...
- (int)pars:(unsigned char)inpt {
    ...
    if ((self.escs.intValue == 0) && (byte == 0x1b)) {
        //NSLog(@"ANSI beg");
        [self.escd appendBytes:&(byte) length:1]; self.escs = @1;
        retn |= ANSI_RETA;
    } else if ((self.escs.intValue == 1) && ((byte == 0x3d) || (byte == 0x5b) || (byte == 0x5d))) {
        if (byte == 0x3d) {
            self.crow = @0; self.ccol = @0;
        ...
        if (byte== 0x5b) {
            [self.escd appendBytes:&(byte) length:1]; self.escs = @2;
        }
        if (byte== 0x5d) {
            [self.escd appendBytes:&(byte) length:1]; self.escs = @20;
        }
        retn |= ANSI_RETB;
    } else if (((self.escs.intValue == 2) || (self.escs.intValue == 20)) && (((byte== ';') || (byte == '?') || (byte == '=') || (byte == '@') || (byte == '<') || (byte == '>')) || (('0' <= byte) && (byte <= '9')))) {
        ...
        retn |= ANSI_RETC;
    } else if (((self.escs.intValue == 2) || (self.escs.intValue == 300)) && ((('a' <= byte) && (byte <= 'z')) || (('A' <= byte) && (byte <= 'Z')))) {
        [self.escd appendBytes:&(byte) length:1];
        const unsigned char *clrs = self.escd.bytes;
        long clen = self.escd.length;
        if (byte == 'l') {
        ...
        self.escd = [[NSMutableData alloc] init];
        retn |= ANSI_RETD;
    } else if (((self.escs.intValue == 20) || (self.escs.intValue == 30)) && (self.escd.length > 2)) {
        const unsigned char *pntr = self.escd.bytes;
        if (self.escs.intValue == 20) {
            self.escs = @30;
        }
        if (byte != 0x07) {
...
- (int)outp:(NSTimer *)objc {
    ...
    while (1) {
            ...
            if (self.mode.intValue == 0) {
                if (self.trys.intValue != 0) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        int retn = [self.vcon show:nil indx:self.vidx.intValue sels:1 back:0 cidx:0 rows:self.rows.intValue cols:self.cols.intValue];
                        if (self.trys.intValue > 0) {
            ...
            if ((stat & ANSI_DATA) != 0) {
                if (self.esci.length > 0) {
                    ...
                    if ((1 < strs.length) && (strs.length < 96)) {
                        ...
                        if (self.vcon != nil) {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                if (pntr[leng] == 0x30) {
                                    NSLog(@"WINT [%@]", strs);//[wino setTitle:strs];
                                }
                                if (pntr[leng] == 0x31) {
                ...
                for (int y = 0; y < plen; ++y) {
                    ...
                    if (self.mode.intValue == 1) { // ansi
                        ...
                        if (byte == 10) { // new
                            NSLog(@"NEWL [%d][%d]", self.crow.intValue, self.ccol.intValue);
                            if ((self.bins.intValue == 1) || (self.bend.intValue == 1)) {
                                int leng = (((int)self.rets.count) - 1);
                        ...
                        if (byte == 13) { // ret
                            NSLog(@"SWAP [%d][%d]", self.crow.intValue, self.ccol.intValue);
                            int indx = [self gidx:0];
                            int rrow = self.crow.intValue, rcol = self.ccol.intValue;
                            if ((self.bins.intValue == 2) || (self.endf.intValue == 3)) {
                                continue;
                            }
                            int flag = 0;
        ...
        if (hlen < self.hist.count) {
            ...
            //NSLog(@"OUTP [%@]", buff);
            dispatch_async(dispatch_get_main_queue(), ^{
                int retn = [self.vcon show:buff indx:self.vidx.intValue sels:1 back:0 cidx:0 rows:self.rows.intValue cols:self.cols.intValue];
                self.trys = @(self.trys.intValue + retn);
            });
        }
        if (self.mode.intValue == 1) {
            if (sync == 1) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    int retn = [self.vcon show:disp indx:self.vidx.intValue sels:2 back:0 cidx:indx rows:self.rows.intValue cols:self.cols.intValue];
                    self.trys = @(self.trys.intValue + retn);
                });
            }
        }
...
- (int)inpt:(NSEvent *)objc indx:(int)indx {
    ...
    //NSLog(@"INPT [%@]", subs);
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.vcon show:subs indx:self.vidx.intValue sels:0 back:0 cidx:cidx rows:self.rows.intValue cols:self.cols.intValue];
    });
    return 0;
}
...
- (void)winz:(NSString *)text wide:(CGFloat)wide high:(CGFloat)high {
    ...
    int fdes = self.mfdn.intValue;
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.tcon sets:text indx:indx wide:wide high:high];
